    ...
    socket.onmessage = async function (message) {
        try {
            ...
            if (data.type === 'auth') {
                ...
                fs.readdir('./conversations/' + chatID, (err, files) => {
                    if (err) {
                    ...
                    // read that file
                    fs.readFile('./conversations/' + chatID + '/' + files[max], (error, data) => {
                        if (error) {
                            socket.send(JSON.stringify({ type: 'error' }))
                            return
                        }
                        ...
                        fs.readFile('./conversations/' + chatID + '/' + files[max - 1], (errored, data) => {
                            if (errored) {
                                socket.send(JSON.stringify({ type: 'error' }))
                                return
                            }
                ...
                // read the folder of that conversation
                fs.readdir('./conversations/' + chatID, (er, files) => {
                    if (er) {
                        socket.send(JSON.stringify({ type: 'error' }))
                        return
                    }
                    ...
                    // read that file
                    fs.readFile('./conversations/' + chatID + '/' + files[max], (err, chatData) => {
                        if (err) {
                            socket.send(JSON.stringify({ type: 'error' }))
                            return
                        }
                        ...
                        // save the file
                        fs.writeFile('./conversations/' + chatID + '/' + files[max], JSON.stringify(chat), (error) => { 
                            if (error) {
                                socket.send(JSON.stringify({ type: 'error' }))
                                return
                            } 
                        ...
                        if (chat.messages.length > 20) {
                            ...
                            fs.writeFile('./conversations/' + chatID + '/' + fileName + '.json', JSON.stringify(newFile), () => {})
                        }
                    })
                })
            } else if (data.type === 'more') {
                fs.readFile('./conversations/' + chatID + '/' + data.number + '.json', (error, data) => {
                    if (error) {
                        socket.send(JSON.stringify({ type: 'error' }))
                        return
                    }
